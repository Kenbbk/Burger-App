...
class BeerService {
    ...
    func execute() {
        ...
        outerLoop: while true {
            ...
            let burgerMenuString = MenuMaker.shared.makeMenuString(for: beers)
            print(burgerMenuString)
            
            let number = try ReadLiner.shared.readNumber(count: beers.count)
            if number == 0 {
                break outerLoop
                
            }
            let selectedIceCream = beers[ number - 1]
